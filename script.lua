...
-- =========================================================================
--                     CRAFTX NIGHT SCRIPT GUI REMIX WITH KEY SYSTEM
-- =========================================================================
task.spawn(function()
    ...
    -- 1. Hide the original UI completely
    if ScaledHolder then
    ...
    pcall(function()
        if writefile and readfile then
            local logoUrl = "https://media.base44.com/images/public/69af19134c5401a25afe3464/2521fa090_CraftX.png"
            local success, logoData = pcall(function()
                return game:HttpGet(logoUrl)
            end)
            if success and logoData then
                writefile(customLogoPath, logoData)
            end
        end
    end)

    local logoID = logoAsset
    if getcustomasset then
        pcall(function()
            logoID = getcustomasset(customLogoPath)
        end)
    end

    -- 3. Core custom ScreenGui
    local CraftXGui = Instance.new("ScreenGui")
    CraftXGui.Name = "CraftXNightGui"
    CraftXGui.ResetOnSpawn = false
    CraftXGui.DisplayOrder = 999999999
    
    local parentFolder = PARENT and PARENT.Parent or (gethui and gethui()) or game:GetService("CoreGui") or LocalPlayer:WaitForChild("PlayerGui")
    CraftXGui.Parent = parentFolder

    -- 4. Notification System
    local NotificationContainer = Instance.new("Frame")
    NotificationContainer.Name = "NotificationContainer"
    NotificationContainer.Size = UDim2.new(0, 250, 0, 100)
    NotificationContainer.Position = UDim2.new(1, -270, 1, -120)
    NotificationContainer.BackgroundTransparency = 1
    NotificationContainer.Parent = CraftXGui

    local function showNotification(text)
        local notif = Instance.new("Frame")
        notif.Size = UDim2.new(1, 0, 0, 40)
        notif.BackgroundColor3 = Color3.fromRGB(13, 26, 18)
        notif.BackgroundTransparency = 0.1
        notif.BorderSizePixel = 0
        notif.Position = UDim2.new(1.2, 0, 1, -50)
        ...
        -- Slide In
        notif:TweenPosition(UDim2.new(0, 0, 1, -50), "Out", "Quart", 0.4, true)
        
        task.delay(2.5, function()
            if notif and notif.Parent then
                notif:TweenPosition(UDim2.new(1.2, 0, 1, -50), "In", "Quart", 0.4, true, function()
                    notif:Destroy()
                end)
            end
        end)
    ...
    local function validateKey(key)
        local cleanKey = tostring(key):gsub("%s+", "")
        
        -- Check if it is a dynamic Key (Starts with CraftX- or CraftXPrm- and is long enough)
        if (string.sub(cleanKey, 1, 7) == "CraftX-" or string.sub(cleanKey, 1, 10) == "CraftXPrm-") and string.len(cleanKey) > 30 then
            ...
            if success and result then
                ...
                if decodeSuccess and data then
                    if data.valid == true or data.status == "success" or data.success == true then
                        if string.sub(cleanKey, 1, 10) == "CraftXPrm-" then
                            _G.PremiumActivated = true
                        end
    ...
    -- Rain Particle Effect Generator Helper
    local function startRainEffect(parentFrame)
        ...
        task.spawn(function()
            while rainActive do
                ...
                drop.Size = UDim2.new(0, 1 + math.random(0, 1), 0, 8 + math.random(4, 12))
                drop.BackgroundColor3 = Color3.fromRGB(34, 197, 94)
                drop.BackgroundTransparency = 0.4 + math.random() * 0.3
                drop.BorderSizePixel = 0
                drop.Position = UDim2.new(math.random(), 0, -0.1, 0)
                drop.ZIndex = parentFrame.ZIndex + 1
                ...
                TweenService:Create(drop, TweenInfo.new(speed, Enum.EasingStyle.Linear), {
                    Position = UDim2.new(drop.Position.X.Scale + 0.04, 0, 1.1, 0),
    ...
    -- Design Gradient Helper
    local function addForestGradient(frame)
        local gradient = Instance.new("UIGradient")
        gradient.Color = ColorSequence.new({
            ColorSequenceKeypoint.new(0.0, Color3.fromRGB(4, 7, 5)),
            ColorSequenceKeypoint.new(0.5, Color3.fromRGB(12, 32, 18)),
            ColorSequenceKeypoint.new(1.0, Color3.fromRGB(4, 7, 5))
        })
        gradient.Rotation = 160
        gradient.Parent = frame
    end

    -- Design Stroke Border Helper
    local function addForestStroke(frame)
        local stroke = Instance.new("UIStroke")
        stroke.Color = Color3.fromRGB(34, 197, 94)
    ...
    -- Design Glow Helper
    local function addForestGlow(frame)
        local glow = Instance.new("ImageLabel")
        glow.Size = UDim2.new(0, 300, 0, 300)
        glow.Position = UDim2.new(0.4, 0, -0.2, 0)
        glow.BackgroundTransparency = 1
        glow.Image = "rbxassetid://9803893322"
    ...
    -- 6. Key Authentication GUI Panel
    local function showKeyGUI()
        local KeyPanel = Instance.new("Frame")
        KeyPanel.Name = "KeyPanel"
        KeyPanel.Size = UDim2.new(0, 380, 0, 240)
        KeyPanel.Position = UDim2.new(0.5, 0, 0.5, 0)
        KeyPanel.AnchorPoint = Vector2.new(0.5, 0.5)
        KeyPanel.BackgroundColor3 = Color3.fromRGB(4, 7, 5)
        KeyPanel.BackgroundTransparency = 0.05
        KeyPanel.BorderSizePixel = 0
        KeyPanel.Parent = CraftXGui

        addForestGradient(KeyPanel)
        addForestStroke(KeyPanel)
        addForestGlow(KeyPanel)
        dragGUI(KeyPanel)
        ...
        kLogo.Size = UDim2.new(0, 40, 0, 40)
        kLogo.Position = UDim2.new(0.5, -20, 0, 18)
        kLogo.BackgroundTransparency = 1
        kLogo.Image = logoID
        kLogo.ZIndex = 3
        kLogo.Parent = KeyPanel
        ...
        kClose.Text = "X"
        kClose.ZIndex = 3
        kClose.Parent = KeyPanel

        local kcCorner = Instance.new("UICorner")
        kcCorner.CornerRadius = UDim.new(0, 5)
        kcCorner.Parent = kClose
        kClose.MouseButton1Click:Connect(function()
            TweenService:Create(keyScale, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.In), {Scale = 0.05}):Play()
            task.wait(0.3)
            CraftXGui:Destroy()
        end)
        ...
        KeyBoxFrame.Size = UDim2.new(0, 280, 0, 32)
        KeyBoxFrame.Position = UDim2.new(0.5, -140, 0, 125)
        KeyBoxFrame.BackgroundColor3 = Color3.fromRGB(13, 26, 18)
        KeyBoxFrame.BackgroundTransparency = 0.4
        KeyBoxFrame.ZIndex = 3
        KeyBoxFrame.Parent = KeyPanel

        local kbfCorner = Instance.new("UICorner")
        kbfCorner.CornerRadius = UDim.new(0, 6)
        kbfCorner.Parent = KeyBoxFrame

        local kbfStroke = Instance.new("UIStroke")
        kbfStroke.Color = Color3.fromRGB(34, 197, 94)
        kbfStroke.Thickness = 1
        kbfStroke.Transparency = 0.7
        kbfStroke.Parent = KeyBoxFrame
        ...
        kSubmit.Size = UDim2.new(0, 135, 0, 32)
        ...
        kSubmit.ZIndex = 3
        kSubmit.Parent = KeyPanel

        local ksCorner = Instance.new("UICorner")
        ksCorner.CornerRadius = UDim.new(0, 6)
        ksCorner.Parent = kSubmit

        local ksStroke = Instance.new("UIStroke")
        ksStroke.Color = Color3.fromRGB(34, 197, 94)
        ksStroke.Thickness = 1
        ksStroke.Transparency = 0.4
        ksStroke.Parent = kSubmit

        -- Get Key Button (Skidnet dashboard)
        local kGetKey = Instance.new("TextButton")
        kGetKey.Size = UDim2.new(0, 135, 0, 32)
        ...
        kGetKey.ZIndex = 3
        kGetKey.Parent = KeyPanel

        local kgCorner = ksCorner:Clone()
        kgCorner.Parent = kGetKey
        local kgStroke = ksStroke:Clone()
        kgStroke.Parent = kGetKey

        -- Buttons Hover Animations
        kSubmit.MouseEnter:Connect(function()
            TweenService:Create(kSubmit, TweenInfo.new(0.2), {BackgroundTransparency = 0.05}):Play()
        end)
        kSubmit.MouseLeave:Connect(function()
            TweenService:Create(kSubmit, TweenInfo.new(0.2), {BackgroundTransparency = 0.2}):Play()
        end)

        kGetKey.MouseEnter:Connect(function()
            TweenService:Create(kGetKey, TweenInfo.new(0.2), {BackgroundTransparency = 0.25, TextColor3 = Color3.fromRGB(255, 255, 255)}):Play()
        end)
        kGetKey.MouseLeave:Connect(function()
            TweenService:Create(kGetKey, TweenInfo.new(0.2), {BackgroundTransparency = 0.5, TextColor3 = Color3.fromRGB(180, 180, 180)}):Play()
        end)
        ...
        kSubmit.MouseButton1Click:Connect(function()
            ...
            -- Helper function to activate premium features
            local function activatePremium()
                -- Make UI Gold!
                local glow = IntroFrame:FindFirstChild("ArmGlow")
                if glow then glow.ImageColor3 = Color3.fromRGB(255, 215, 0) end
                
                -- Recolor brand elements
                for _, v in pairs(CraftXGui:GetDescendants()) do
                    if v:IsA("Frame") and v.BackgroundColor3 == Color3.fromRGB(34, 197, 94) then
                        v.BackgroundColor3 = Color3.fromRGB(255, 215, 0)
                    elseif v:IsA("TextLabel") and v.TextColor3 == Color3.fromRGB(34, 197, 94) then
                        v.TextColor3 = Color3.fromRGB(255, 215, 0)
                    end
                end
                
                -- Find the premium tab button and show it (it will be created later in the code)
                local pBtn = Sidebar:FindFirstChild("PremiumTabBtn")
                if pBtn then pBtn.Visible = true end
                
                game.StarterGui:SetCore("SendNotification", {
                    Title = "💎 Premium Unlocked 💎",
                    Text = "Welcome to CraftX Premium!",
                    Duration = 5,
                })
            end

            local valid, msg = pcall(function() return validateKey(enteredKey) end)
            if valid and type(msg) == "table" then
                -- This shouldn't happen, but just in case
                valid = false
            end
            
            -- Wait, if valid is true but the function returned false, msg will actually be the first return value (which is true/false)
            -- Let's do it properly:
            local pcallSuccess, ret1, ret2 = pcall(function() return validateKey(enteredKey) end)
            
            if pcallSuccess and ret1 == true then
                valid = true
            else
                valid = false
                if not pcallSuccess then
                    msg = "Error: " .. tostring(ret1)
                else
                    msg = tostring(ret2)
                end
            end
            ...
            if valid then
                if _G.PremiumActivated then
                    activatePremium()
                end
                -- Save key
                pcall(function()
                    if writefile then
                        writefile(keySavePath, enteredKey)
                    end
                end)
                
                showNotification("Success! Loading CraftX Night Script...")
                kSubmit.Text = "Success!"
                
                -- Close key UI
                stopRain()
                TweenService:Create(keyScale, TweenInfo.new(0.4, Enum.EasingStyle.Back, Enum.EasingDirection.In), {Scale = 0.05}):Play()
                task.wait(0.4)
                KeyPanel:Destroy()
                ...
                kSubmit.Text = "Submit Key"
                showNotification("Verification failed: " .. msg)
                
                -- Shake input box
                local origPos = KeyBoxFrame.Position
                for i = 1, 3 do
                    KeyBoxFrame.Position = origPos + UDim2.new(0, -6, 0, 0)
                    task.wait(0.04)
                    KeyBoxFrame.Position = origPos + UDim2.new(0, 6, 0, 0)
                    task.wait(0.04)
                end
                KeyBoxFrame.Position = origPos
            end
        end)
    end
    ...
    loadMainGUI = function()
        ...
        MainPanel.BackgroundColor3 = Color3.fromRGB(4, 7, 5)
        MainPanel.BackgroundTransparency = 0.05
        MainPanel.BorderSizePixel = 0
        MainPanel.Parent = CraftXGui

        addForestGradient(MainPanel)
        addForestStroke(MainPanel)
        addForestGlow(MainPanel)
        dragGUI(MainPanel)
        ...
        sep.Position = UDim2.new(1, -1, 0, 12)
        sep.BackgroundColor3 = Color3.fromRGB(34, 197, 94)
        ...
        logoImg.Position = UDim2.new(0.5, -22, 0, 15)
        ...
        execTabBtn.MouseButton1Click:Connect(function()
            ...
            if ScriptsPanel then ScriptsPanel.Visible = false end
            if PremiumPanel then PremiumPanel.Visible = false end
        end)
        
        -- Scripts Tab Button
        local scriptsTabBtn = Instance.new("TextButton")
        scriptsTabBtn.Size = UDim2.new(0, 60, 0, 24)
        scriptsTabBtn.Position = UDim2.new(0.5, -30, 0, 130)
        scriptsTabBtn.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
        scriptsTabBtn.Font = Enum.Font.Jura
        ...
        scCorner.Parent = scriptsTabBtn
        
        -- Premium Tab Button
        premiumTabBtn = Instance.new("TextButton")
        premiumTabBtn.Name = "PremiumTabBtn"
        premiumTabBtn.Size = UDim2.new(0, 60, 0, 24)
        premiumTabBtn.Position = UDim2.new(0.5, -30, 0, 165)
        premiumTabBtn.BackgroundColor3 = Color3.fromRGB(255, 215, 0)
        premiumTabBtn.Font = Enum.Font.Jura
        premiumTabBtn.TextColor3 = Color3.fromRGB(0, 0, 0)
        premiumTabBtn.TextSize = 11
        premiumTabBtn.Text = "Premium"
        premiumTabBtn.Parent = Sidebar
        premiumTabBtn.Visible = false -- Hidden until premium activated
        local pCorner = Instance.new("UICorner")
        pCorner.CornerRadius = UDim.new(0, 4)
        pCorner.Parent = premiumTabBtn
        
        -- Add global references for the panels
        ScriptsPanel = Instance.new("Frame")
        PremiumPanel = Instance.new("Frame")
        
        scriptsTabBtn.MouseButton1Click:Connect(function()
            MainPanel.Visible = false
            ExecutorPanel.Visible = false
            ScriptsPanel.Visible = true
            PremiumPanel.Visible = false
        end)
        ...
        settingsBtn.MouseButton1Click:Connect(function()
            task.spawn(function()
                ...
                -- Hide the Custom GUI Panels so the Infinite Yield settings are visible
                if MainGui then MainGui.Visible = false end
                
                -- The showSettings function is available at the top of the file!
        ...
        youtubeBtn.BackgroundTransparency = 0.2
        youtubeBtn.Text = "YT"
        ...
        dsCorner.CornerRadius = UDim.new(1, 0)
        dsCorner.Parent = discordBtn
        ...
        HeaderFrame.Size = UDim2.new(1, -85, 0, 50)
        HeaderFrame.Position = UDim2.new(0, 75, 0, 10)
        ...
        -- Close script GUI button
        local closeGuiBtn = Instance.new("TextButton")
        closeGuiBtn.Size = UDim2.new(0, 24, 0, 24)
        ...
        closeGuiBtn.Text = "X"
        closeGuiBtn.Parent = HeaderFrame

        local cgCorner = Instance.new("UICorner")
        cgCorner.CornerRadius = UDim.new(0, 6)
        cgCorner.Parent = closeGuiBtn
        closeGuiBtn.MouseEnter:Connect(function()
            TweenService:Create(closeGuiBtn, TweenInfo.new(0.2), {BackgroundTransparency = 0.5}):Play()
        end)
        closeGuiBtn.MouseLeave:Connect(function()
            TweenService:Create(closeGuiBtn, TweenInfo.new(0.2), {BackgroundTransparency = 0.8}):Play()
        end)
        closeGuiBtn.MouseButton1Click:Connect(function()
            TweenService:Create(uiScale, TweenInfo.new(0.3, Enum.EasingStyle.Back, Enum.EasingDirection.In), {Scale = 0.05}):Play()
            task.wait(0.3)
            CraftXGui:Destroy()
        end)
        ...
        SearchContainer.Position = UDim2.new(1, -215, 0, 11)
        SearchContainer.BackgroundColor3 = Color3.fromRGB(13, 26, 18)
        ...
        scStroke.Color = Color3.fromRGB(34, 197, 94)
        ...
        TabContainer.Size = UDim2.new(1, -85, 0, 30)
        TabContainer.Position = UDim2.new(0, 75, 0, 65)
        ...
        -- Command Pages Container
        local ContentContainer = Instance.new("Frame")
        ContentContainer.Size = UDim2.new(1, -85, 1, -115)
        ContentContainer.Position = UDim2.new(0, 75, 0, 105)
        ContentContainer.BackgroundTransparency = 1
        ContentContainer.ZIndex = 2
        ContentContainer.Parent = MainPanel

        -- Command Bar for direct console execution
        local ConsoleBarFrame = Instance.new("Frame")
        ConsoleBarFrame.Size = UDim2.new(1, 0, 0, 32)
        ConsoleBarFrame.Position = UDim2.new(0, 0, 1, -35)
        ConsoleBarFrame.BackgroundColor3 = Color3.fromRGB(13, 26, 18)
        ConsoleBarFrame.BackgroundTransparency = 0.5
        ConsoleBarFrame.Visible = false
        ConsoleBarFrame.Parent = ContentContainer

        local cbCorner = Instance.new("UICorner")
        cbCorner.CornerRadius = UDim.new(0, 6)
        cbCorner.Parent = ConsoleBarFrame
        local cbStroke = Instance.new("UIStroke")
        cbStroke.Color = Color3.fromRGB(34, 197, 94)
        cbStroke.Thickness = 1
        cbStroke.Transparency = 0.7
        cbStroke.Parent = ConsoleBarFrame

        local ConsoleBox = Instance.new("TextBox")
        ConsoleBox.Size = UDim2.new(1, -60, 1, 0)
        ConsoleBox.Position = UDim2.new(0, 10, 0, 0)
        ConsoleBox.BackgroundTransparency = 1
        ConsoleBox.Font = Enum.Font.Code
        ConsoleBox.PlaceholderText = "Execute command string (e.g. flyspeed 80)..."
        ConsoleBox.PlaceholderColor3 = Color3.fromRGB(130, 130, 130)
        ConsoleBox.TextColor3 = Color3.fromRGB(255, 255, 255)
        ConsoleBox.TextSize = 13
        ConsoleBox.Text = ""
        ConsoleBox.TextXAlignment = Enum.TextXAlignment.Left
        ConsoleBox.Parent = ConsoleBarFrame

        local ConsoleRunBtn = Instance.new("TextButton")
        ConsoleRunBtn.Size = UDim2.new(0, 50, 1, 0)
        ...
        ConsoleRunBtn.Parent = ConsoleBarFrame

        local crCorner = Instance.new("UICorner")
        crCorner.CornerRadius = UDim.new(0, 6)
        crCorner.Parent = ConsoleRunBtn

        ConsoleBox.FocusLost:Connect(function(enterPressed)
            if enterPressed and ConsoleBox.Text ~= "" then
                execCmd(ConsoleBox.Text)
                showNotification("Executed: " .. ConsoleBox.Text)
                ConsoleBox.Text = ""
            end
        end)
        ConsoleRunBtn.MouseButton1Click:Connect(function()
        ...
        -- Define tabs and scrollframes
        local tabNames = {"Player", "Movement", "Combat", "Visuals", "Utility", "Console"}
        local tabs = {}
        local scrollFrames = {}
        ...
        SearchResultsScroll.ScrollBarImageColor3 = Color3.fromRGB(34, 197, 94)
        SearchResultsScroll.Visible = false
        SearchResultsScroll.Parent = ContentContainer

        local srGrid = Instance.new("UIGridLayout")
        srGrid.CellSize = UDim2.new(0, 122, 0, 36)
        srGrid.CellPadding = UDim2.new(0, 6, 0, 6)
        srGrid.SortOrder = Enum.SortOrder.Name
        srGrid.Parent = SearchResultsScroll
        ...
        local function selectTab(tabName)
            ...
            for name, scrollFrame in pairs(scrollFrames) do
                if name == tabName then
                    if name == "Console" then
                        scrollFrame.Size = UDim2.new(1, 0, 1, -45)
                        ConsoleBarFrame.Visible = true
                    else
        ...
        for idx, name in ipairs(tabNames) do
            ...
            ts.Color = Color3.fromRGB(34, 197, 94)
            ts.Thickness = 1
            ts.Transparency = 0.5
            ts.Enabled = false
            ts.Parent = tabBtn
            ...
            sf.ScrollBarImageColor3 = Color3.fromRGB(34, 197, 94)
            sf.Visible = false
            sf.Parent = ContentContainer

            local grid = srGrid:Clone()
            grid.Parent = sf

            scrollFrames[name] = sf
        end

        -- Categorization & Toggle Logic
        local commandCategories = {
            Player = {
                walkspeed = true, ws = true, jumppower = true, jp = true, hipheight = true, 
                respawn = true, re = true, refresh = true, ref = true, kill = true, 
                loopkill = true, view = true, unview = true, goto = true, to = true, 
                bring = true, spectate = true, unspectate = true, age = true, naked = true
            },
            Movement = {
                fly = true, unfly = true, flyspeed = true, cframefly = true, cfly = true, 
                uncframefly = true, uncfly = true, noclip = true, clip = true, float = true, 
                unfloat = true, swim = true, unswim = true, infinitejump = true, infjump = true, 
                uninfinitejump = true, uninfjump = true, clicktp = true, walkfling = true, 
                unwalkfling = true, flyfling = true, unflyfling = true
            },
            Combat = {
                fling = true, unfling = true, loopfling = true, unloopfling = true, 
                kill = true, loopkill = true, unloopkill = true, void = true
            },
            Visuals = {
                esp = true, noesp = true, chams = true, nochams = true, xray = true, 
                noxray = true, fullbright = true, fb = true, unfullbright = true, 
                fps = true, fpscap = true
            },
            Utility = {
                rejoin = true, rj = true, serverhop = true, shop = true, copy = true, 
                clipboard = true, chat = true, clear = true, log = true, logs = true, 
                dex = true, explorer = true, spy = true, remotespy = true, btools = true, 
                backpack = true
            }
        }
        ...
        local togglePairs = {
            ...
            flyfling = "unflyfling",
            loopfling = "unloopfling",
            loopkill = "unloopkill",
            view = "unview",
            spectate = "unspectate"
        }

        local reverseToggles = {}
        for k, v in pairs(togglePairs) do
            reverseToggles[v] = k
        end

        local customButtonNames = {
            fly = {on = "flying on", off = "flying off"},
            noclip = {on = "noclip on", off = "noclip off"},
            float = {on = "float on", off = "float off"},
            swim = {on = "swim on", off = "swim off"},
            infinitejump = {on = "infjump on", off = "infjump off"},
            esp = {on = "esp on", off = "esp off"},
            chams = {on = "chams on", off = "chams off"},
            xray = {on = "xray on", off = "xray off"},
            fullbright = {on = "fullbright on", off = "fullbright off"},
            walkfling = {on = "walkfling on", off = "walkfling off"},
            flyfling = {on = "flyfling on", off = "flyfling off"},
            loopfling = {on = "loopfling on", off = "loopfling off"},
            loopkill = {on = "loopkill on", off = "loopkill off"},
            view = {on = "viewing on", off = "viewing off"},
            spectate = {on = "spectate on", off = "spectate off"}
        }
        ...
        local defaultToggles = {
            walkspeed = {on = "walkspeed 100", off = "walkspeed 16"},
            ws = {on = "ws 100", off = "ws 16"},
            jumppower = {on = "jumppower 150", off = "jumppower 50"},
            jp = {on = "jp 150", off = "jp 50"},
            flyspeed = {on = "flyspeed 100", off = "flyspeed 50"}
        }

        local function updateButtonState(btn, state, text)
            btn:SetAttribute("ActiveState", state)
            if state then
                btn.BackgroundColor3 = Color3.fromRGB(34, 197, 94)
                btn.BackgroundTransparency = 0.25
                btn.TextColor3 = Color3.fromRGB(255, 255, 255)
                btn.Text = text or btn:GetAttribute("DisplayName")
                local stroke = btn:FindFirstChildOfClass("UIStroke")
                if stroke then stroke.Color = Color3.fromRGB(34, 197, 94) stroke.Transparency = 0.2 end
                local dot = btn:FindFirstChild("ActiveDot")
                if dot then dot.Visible = true end
            else
                btn.BackgroundColor3 = Color3.fromRGB(13, 26, 18)
                btn.BackgroundTransparency = 0.5
                btn.TextColor3 = Color3.fromRGB(170, 170, 170)
                btn.Text = text or btn:GetAttribute("DisplayName")
                local stroke = btn:FindFirstChildOfClass("UIStroke")
                if stroke then stroke.Color = Color3.fromRGB(34, 197, 94) stroke.Transparency = 0.85 end
                local dot = btn:FindFirstChild("ActiveDot")
        ...
        local function flashButton(btn)
            ...
            task.delay(0.25, function()
                if btn and btn.Parent then
                    ...
                    btn.BackgroundTransparency = origTrans
                    btn.TextColor3 = origText
                end
            end)
        end

        local function runCommand(cmdName, btn)
            cmdName = cmdName:lower()
            
            if togglePairs[cmdName] then
                if activeToggles[cmdName] then
                    execCmd(togglePairs[cmdName])
                    activeToggles[cmdName] = false
                    local offText = customButtonNames[cmdName] and customButtonNames[cmdName].off or (cmdName .. " off")
                    updateButtonState(btn, false, offText)
                else
                    execCmd(cmdName)
                    activeToggles[cmdName] = true
                    local onText = customButtonNames[cmdName] and customButtonNames[cmdName].on or (cmdName .. " on")
                    updateButtonState(btn, true, onText)
                end
            elseif defaultToggles[cmdName] then
                if activeToggles[cmdName] then
                    execCmd(defaultToggles[cmdName].off)
                    activeToggles[cmdName] = false
                    updateButtonState(btn, false, cmdName .. " off")
                else
                    execCmd(defaultToggles[cmdName].on)
                    activeToggles[cmdName] = true
                    updateButtonState(btn, true, cmdName .. " on")
                end
            else
                execCmd(cmdName)
                flashButton(btn)
        ...
        local function createCommandButton(cmd, parentScroll)
            ...
            local isToggle = togglePairs[cmd.NAME:lower()] ~= nil or defaultToggles[cmd.NAME:lower()] ~= nil
            
            if isToggle then
                displayName = customButtonNames[cmd.NAME:lower()] and customButtonNames[cmd.NAME:lower()].off or (cmd.NAME .. " off")
            end
            ...
            btn:SetAttribute("CommandName", cmd.NAME)
            btn:SetAttribute("DisplayName", displayName)
            ...
            btn.MouseEnter:Connect(function()
                if not btn:GetAttribute("ActiveState") then
                    TweenService:Create(btn, TweenInfo.new(0.2), {BackgroundTransparency = 0.35, TextColor3 = Color3.fromRGB(255, 255, 255)}):Play()
                    TweenService:Create(stroke, TweenInfo.new(0.2), {Transparency = 0.5}):Play()
                end
            end)
            btn.MouseLeave:Connect(function()
                if not btn:GetAttribute("ActiveState") then
                    TweenService:Create(btn, TweenInfo.new(0.2), {BackgroundTransparency = 0.5, TextColor3 = Color3.fromRGB(170, 170, 170)}):Play()
                    TweenService:Create(stroke, TweenInfo.new(0.2), {Transparency = 0.85}):Play()
                end
            end)

            btn.MouseButton1Click:Connect(function()
                runCommand(cmd.NAME, btn)
            end)

            table.insert(allButtons, btn)
        end

        -- Process commands from Infinite Yield 'cmds' table
        for _, cmd in pairs(cmds) do
            local name = cmd.NAME:lower()
            
            -- Filter out "off" commands so we only have one button per toggle
            if not reverseToggles[name] then
                local category = "Console"
                
                for cat, mapping in pairs(commandCategories) do
                ...
                local parentScroll = scrollFrames[category]
                if parentScroll then
                    createCommandButton(cmd, parentScroll)
                end
            end
        end

        -- Dynamically update CanvasSize of ScrollFrames
        for name, sf in pairs(scrollFrames) do
            local grid = sf:FindFirstChildOfClass("UIGridLayout")
            if grid then
                local count = 0
                ...
                sf.CanvasSize = UDim2.new(0, 0, 0, rows * 42 + 20)
            end
        end
        ...
        SearchBox:GetPropertyChangedSignal("Text"):Connect(function()
            ...
            if txt == "" then
                SearchResultsScroll.Visible = false
                local currentScroll = scrollFrames[activeTab]
                if currentScroll then
                    currentScroll.Visible = true
                end
            else
                ...
                for _, btn in pairs(allButtons) do
                    local name = btn:GetAttribute("CommandName"):lower()
                    if string.find(name, txt) then
                        local clone = btn:Clone()
                        clone.Parent = SearchResultsScroll
                        
                        local stroke = clone:FindFirstChildOfClass("UIStroke")
                        
                        clone.MouseEnter:Connect(function()
                            if not clone:GetAttribute("ActiveState") then
                                TweenService:Create(clone, TweenInfo.new(0.2), {BackgroundTransparency = 0.35, TextColor3 = Color3.fromRGB(255, 255, 255)}):Play()
                                if stroke then TweenService:Create(stroke, TweenInfo.new(0.2), {Transparency = 0.5}):Play() end
                            end
                        end)
                        clone.MouseLeave:Connect(function()
                            if not clone:GetAttribute("ActiveState") then
                                TweenService:Create(clone, TweenInfo.new(0.2), {BackgroundTransparency = 0.5, TextColor3 = Color3.fromRGB(170, 170, 170)}):Play()
                                if stroke then TweenService:Create(stroke, TweenInfo.new(0.2), {Transparency = 0.85}):Play() end
                            end
                        end)
                        clone.MouseButton1Click:Connect(function()
                            runCommand(btn.Name, clone)
                            local isAct = clone:GetAttribute("ActiveState")
                            updateButtonState(btn, isAct, clone.Text)
                            activeToggles[btn.Name:lower()] = isAct
                        end)
                ...
                local rows = math.ceil(count / 4)
                SearchResultsScroll.CanvasSize = UDim2.new(0, 0, 0, rows * 42 + 20)
            end
        end)
    ...
    pcall(function()
        if readfile and isfile and isfile(keySavePath) then
            local savedKey = readfile(keySavePath)
    ...
    local function playCustomIntro(callback)
        ...
        -- V2 Intro Animation (Dynamic Spin & Scale)
        Logo.Size = UDim2.new(0, 0, 0, 0)
        Logo.Position = UDim2.new(0.5, 0, 0.3, 0)
        Logo.Rotation = -180
        
        local logoTween = TweenService:Create(Logo, TweenInfo.new(1.2, Enum.EasingStyle.Elastic, Enum.EasingDirection.Out), {
        ...
        -- Flash background slightly green
        local flash = TweenService:Create(IntroFrame, TweenInfo.new(0.3, Enum.EasingStyle.Sine), {BackgroundColor3 = Color3.fromRGB(15, 40, 20)})
        flash:Play()
        flash.Completed:Connect(function()
            TweenService:Create(IntroFrame, TweenInfo.new(0.8, Enum.EasingStyle.Sine), {BackgroundColor3 = Color3.fromRGB(15, 15, 15)}):Play()
        end)

        WelcomeText.Position = UDim2.new(0.1, -500, 0.45, 0)
        TweenService:Create(WelcomeText, TweenInfo.new(0.8, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = UDim2.new(0.1, 0, 0.45, 0)}):Play()
        task.wait(0.2)

        SubText.Position = UDim2.new(0.1, 500, 0.55, 0)
        TweenService:Create(SubText, TweenInfo.new(0.8, Enum.EasingStyle.Back, Enum.EasingDirection.Out), {Position = UDim2.new(0.1, 0, 0.55, 0)}):Play()
        
        -- Let them read it
        task.wait(2.2)
        
        -- 5. Elements Zip Out
        TweenService:Create(Logo, TweenInfo.new(0.6, Enum.EasingStyle.Back, Enum.EasingDirection.In), {Position = UDim2.new(0.5, -70, -0.5, 0), Size = UDim2.new(0,0,0,0)}):Play()
        task.wait(0.1)
        TweenService:Create(WelcomeText, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.In), {Position = UDim2.new(0.1, -500, 0.45, 0)}):Play()
        TweenService:Create(SubText, TweenInfo.new(0.5, Enum.EasingStyle.Back, Enum.EasingDirection.In), {Position = UDim2.new(0.1, 500, 0.55, 0)}):Play()
